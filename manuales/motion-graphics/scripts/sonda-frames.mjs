 ...
 * avisar. Pasó de verdad: al comprobar un cambio en los chips del 006 —que viven
 * en [1633,1767]— la sonda muestreaba 1606 y 1874 y pasaba por encima; el
 * resultado fue «no cambia nada», que era falso. Los 8 frames de reparto sirven
 * para detectar REGRESIONES en toda la pieza; para PROBAR que un cambio hace lo
 * que dice, localiza primero en qué tomas está la pieza afectada y apunta ahí
 * con `--frames`.
 *
 ...
 * núcleo o un theme, hay que mirar las diecisiete composiciones, no una.
 *
...
async function main() {
  ...
  if (!nombre || nombre.indexOf("--") === 0) {
    console.error("uso: sonda-frames.mjs <nombre-tanda> [--muestras N] [--escala E] [--comps A,B] [--frames a,b]");
    process.exit(1);
  }
  ...
  console.log(`\n${hechos} frames en ${path.relative(root, salida)}${fallos ? ` · ${fallos} fallos` : ""}\n`);
