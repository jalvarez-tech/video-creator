...
description: >-
  El SEGUNDO MOTOR del sistema "video-creator": HyperFrames (heygen-com/hyperframes),
  que renderiza vídeo desde HTML+GSAP en local, gratis y sin API key, como
  alternativa al motor Remotion —que sigue siendo el de por defecto—. Úsalo cuando
  una pieza se elija explícitamente para este motor, no por costumbre. Cubre qué es
  HyperFrames y qué NO es (no genera avatares, no renderiza en la nube salvo que se
  pida y se pague), la tabla de decisión entre los dos motores, la instalación sin
  romper las skills del repo (ojo: hyperframes trae una skill llamada
  `motion-graphics` que colisiona con la tuya), el contrato de composición
  (`data-*` en SEGUNDOS, `class="clip"` con su ventana temporal, una timeline
  pausada en `window.__timelines`, determinismo), el puente de marca
  (`marca-a-css.mjs`: la marca se GENERA desde `src/marcas/<canal>.ts`, no se copia),
  las dos puertas obligatorias (`revisar-hf.mjs` + `hyperframes check`) y el
  diccionario Remotion→HyperFrames con las tres cosas que NO traducen (`spring()`,
  `<Freeze>` y las tablas de avances medidas contra San Francisco; la cámara
  virtual SÍ traduce). Los recursos
  siguen siendo de HeyGen: el avatar se genera con `heygen.py` y entra como un
  `<video>`. Triggers: "hyperframes", "hyperfames", "hypergrames", "otro motor",
  "segundo motor", "motor alternativo", "render en HTML", "vídeo con HTML y CSS",
  "sin React", "npx hyperframes", "check de contraste", "quitar el fondo del avatar",
  "remove-background", "montar esta pieza en hyperframes", "todo en HeyGen".
user-invocable: true
metadata:
  type: reference
---
...
Root: **`/Users/nicecode/Work/jalvarez/video-creator`**. Este skill es hermano de [edicion-video](../edicion-video/SKILL.md), no sustituto: aporta un motor más, no un sistema paralelo. La narrativa, la marca, el b-roll, el sonido y el avatar **no cambian** — cambia dónde se dibuja.

Verificado ejecutando el CLI **v0.7.107** en este Mac el 2026-08-13: composición 9:16 renderizada a 1080×1920, 25 fps, 200 frames, con audio del banco de `sonido/`. Lo que no se ha ejecutado va marcado como **sin verificar**.
...
| «hace avatares» | No. Su CLI **no tiene comando de avatar** (`hyperframes --help`, v0.7.107) y **ningún bloque del registry genera uno** — los dos que nombran «avatar» (`yt-comment-card`, `yt-lower-third`) usan una foto de perfil como imagen. El avatar sigue saliendo de [`heygen.py`](../edicion-video/heygen.md) y entra como un `<video>`. |
...
| El plan de gráficos **como dato** (`Plan` → `PistaGraficos`) | **Remotion** | El intérprete y las 37 piezas están en JSX. HF no las tiene. |
...
El CLI no se instala: se invoca con `npx`. Lo único que hay que comprobar una vez:

```bash
npx hyperframes doctor
```

Este Mac ya cumple: Node 25, FFmpeg 8.1.2, Chrome, whisper-cpp. Kokoro y MusicGen salen ✗ y **no hacen falta** (son fallbacks locales de voz y música; la voz de este sistema es ElevenLabs o HeyGen).

### La colisión de `motion-graphics`, y cómo se resuelve

> ⛔ **NUNCA corras `npx skills add heygen-com/hyperframes` dentro de este repo.**
> HyperFrames trae 20 skills y una se llama **`motion-graphics`** — el mismo nombre que la tuya. **Reproducido en un sandbox:** el instalador **borra** el symlink `.claude/skills/motion-graphics → ../../manuales/motion-graphics`, que está **versionado en git** (`git status` lo enseña como ` D`), y deja en su sitio una **copia real** de la de HyperFrames. `manuales/motion-graphics/` sobrevive en disco pero **deja de ser la skill que se carga**, y el fallo se nota tarde: los gráficos empiezan a salir con otro criterio.
...
> Si ya pasó: `git checkout -- .claude/skills/motion-graphics` y borra la entrada del lockfile.
...
Instala en un temporal (para que su `skills-lock.json` muera ahí y no toque el tuyo), copia a `.claude/skills/heygen-<nombre>/` y reescribe el frontmatter con el nombre prefijado y **la regla de enrutado delante de la descripción**. Las copias van al `.gitignore` — son de terceros y se reponen corriendo el script otra vez, misma decisión que las de ElevenLabs.
...
```
Deja `proyectos/008/hf/` con `index.html` (la plantilla ya renderizada y mirada), `marca.css` **generado**, GSAP vendorizado y `assets/`.
...
**4. Los recursos son los de siempre.** Avatar → `heygen.py`. B-roll → la regla de honestidad de [director-video §3h](../director-video/SKILL.md) (lo real se **trae** con `bancos.py`, lo que no existe se **genera** con `grok.py`, y un concepto sin referente filmable **no es b-roll, es gráfico**). SFX → el banco de `sonido/` con el criterio de [diseno-sonoro](../diseno-sonoro/SKILL.md). Todo va a `proyectos/NNN/hf/assets/`. **Ninguna URL en el HTML**: un proyecto tiene que poder re-renderizarse dentro de un año.
...
```bash
npx hyperframes snapshot proyectos/008/hf --at 0,2,4,6
npx hyperframes preview proyectos/008/hf     # Studio, para revisar con el usuario
```

**7. Render por el wrapper, nunca a pelo.**
```bash
bash manuales/motor-hyperframes/scripts/render-hf.sh 008            # prueba
bash manuales/motor-hyperframes/scripts/render-hf.sh 008 --final    # tras el OK
```
A pelo, `render` escribe en `renders/<nombre>_<timestamp>.mp4` **relativo al CWD** y se salta las dos puertas. El wrapper las corre, apaga la telemetría y deja el MP4 donde el repo los guarda.
...
Lee **el mismo fichero que lee Remotion** (`src/marcas/<canal>.ts`) y emite las custom properties. Cambiar `luxur.ts` y volver a correrlo mueve **los dos motores a la vez**. Un `#FF5500` escrito a mano en el `<style>` deshace en un archivo lo que costó sacar la marca del motor — y lo hace en silencio, porque el vídeo sale bien mientras el canal sea Luxur. **`revisar-hf.mjs` falla si encuentra un hex a pelo.**

Dos cosas que el script decide y conviene entender:

- **`--acento-texto`.** El acento **es una tinta de TEXTO** del dialecto editorial (`TintaNoticia = "tinta" | "suave" | "blanco" | "acento" | "resalte" | "papel"`, `noticias/dialecto.ts:87`): con él se pinta la cifra héroe de las tomas `cifra` y la primera barra de las de datos. Y sobre el papel mide **2.62:1**, por debajo del 3:1 que exige la puerta de contraste para texto grande. El script calcula una variante oscurecida contra el papel real del canal (para Luxur: `#eb4e00`) y falla si esa pareja no se puede leer ni oscureciéndola.
  ⚠️ **Esto no es un problema de HyperFrames: es un problema de la marca que HyperFrames ha detectado**, y afecta a piezas ya publicadas en Remotion. Aquí queda corregido; **en Remotion sigue sin corregir**, porque cambiar `luxur.ts` movería el look de los vídeos 004-007 ya renderizados. Decisión pendiente, no olvido.
...
8. ¿Pasan **las dos** puertas? ¿Y el `check` enseña **9 muestras de layout y 21 de contraste**, no `0 sample(s)`? (Cero muestras no es cero problemas: es un lint en rojo que apagó las pasadas profundas.)
...
- **No** porta las piezas existentes (006, 007). Existe `/remotion-to-hyperframes` oficial, pero es porte **manual y de una sola dirección**: deja un HTML huérfano que ya no se regenera desde el plan.
- **No** construye el **emisor doble** (un plan → los dos motores). Es viable —`Montadores<R,C,N>` ya es genérico en el nodo de salida, y el 61 % del pipeline declarativo es dato puro— pero son ~4.900 líneas nuevas y **sin paridad de píxel**. Es un proyecto de motor, no una skill. Antes de plantearlo hacen falta piezas reales aquí.
