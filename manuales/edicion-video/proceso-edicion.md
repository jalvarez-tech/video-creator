...
```bash
ffprobe -v error \
  -show_entries format=duration:stream=index,codec_type,codec_name,width,height,r_frame_rate,channels,sample_rate \
  -of default=noprint_wrappers=1 "proyectos/001/original.mp4"
```
**Claude entrega:**
...
> **Prompt para copiar:**
> «Te paso un vídeo base en `proyectos/001/original.mp4`. Antes de editar: 1) localiza el original, 2) dime duración, resolución, fps y audio, 3) dime si hay carpeta de proyecto, 4) qué archivos existen, 5) qué falta para editar. No exportes todavía.»
...
**Acción (Claude):**
```bash
# a) Evidencias sin renderizar: cuánto se cortaría y dónde
auto-editor "proyectos/001/original.mp4" --edit "audio:threshold=0.06" --margin 0.2s --preview

# b) Timeline para DaVinci (no renderiza vídeo, genera FCPXML)
auto-editor "proyectos/001/original.mp4" --export resolve --margin 0sec \
  --edit "audio:threshold=0.06" \
  -o "proyectos/001/corte-auto-editor/001-resolve.fcpxml"
```
**Claude entrega:**
...
> **Prompt:** «Corta los silencios de `proyectos/001/original.mp4` con nuestro comando. Devuélveme: 1) archivo y ruta, 2) duración original vs. cortada, 3) timestamps de cortes dudosos, 4) siguiente acción recomendada. No sigas editando.»
...
**Acción (Claude):** transcribir con tiempos → `proyectos/001/transcripcion.json`.
```bash
# Wrapper de whisper.cpp (ya instalado): vídeo/audio → JSON con tiempos
bash manuales/edicion-video/scripts/transcribir.sh \
  "proyectos/001/original.mp4" "proyectos/001/transcripcion.json" es
```
...
```bash
cd remotion
# Prueba ligera del tramo en 720p
npx remotion render <Plantilla> ../proyectos/001/pruebas-720p/tramo-00-10.mp4 --frames=0-299 --scale=0.6666667
```
...
## 🔧 Transcripción — instalada ✅

El **Paso 3** usa **whisper.cpp** (nativo, rápido en Apple Silicon) mediante el wrapper `scripts/transcribir.sh`.
- Binario: `whisper-cli` (whisper-cpp 1.9.1, brew) · Modelo por defecto: `archivos/whisper/ggml-small.bin`.
- Uso: `bash manuales/edicion-video/scripts/transcribir.sh <entrada> <salida.json> [idioma] [modelo.bin]`.
- **Más precisión:** descarga `ggml-medium.bin` o `ggml-large-v3.bin` en `archivos/whisper/` y pásalo como 4º argumento.
- **Subtítulos karaoke** (tiempos por palabra): cambia `-oj` por `-ojf` en el script.

> El proceso completo (pasos 1–7) es ya 100% ejecutable.
